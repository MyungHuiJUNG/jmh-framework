...
const canGetCode = computed(() => permissionRoleStore.hasPermission(FlowRoleCode.CODE_ROLE.READ_SYSTEM_CODE));
...
function getDepth(node, currentDepth = 0) {
  if (!node.parentEntityId || node.parentEntityId <= 0) {
    return currentDepth;
  }
...
function postCode() {
  ...
  // 이동된 노드들의 이전 부모에서 자식 노드들의 orderNumber 재조정
  adjustOrderNumbersInOriginalParents();

  if (putParams.length > 0) {
    putParams.sort((a, b) => (a.orderNumber || 0) - (b.orderNumber || 0));
...
<template>
  <div class="a-border full-height fit column q-pa-xs">
    <a-title-bar title="시스템 코드 목록" class="col-auto full-width q-mb-xs" />
    <tabulator-grid
      ref="childRef"
      :rows="rows"
      :columns="columns"
