        ...
        const sortDataByOrderNumber = (data) => {
          return data
            .map((item) => {
              ...
              const isActive = permissionStore.menuIds.includes(entityId);
...
function postMenusByPermissions() {
  ...
  permissionApi
    .postMenusByPermission(param, permissionStore.entityId)
    .then(() => {
      menuStore.getMenus(authStore.roleGroup.entityId);
    })
    .catch((error) => {
      handleApiError(error);
    });
...
watch(
  () => permissionStore.menuIds,
  () => {
    getMenus(); // menuIds가 변경되면 메뉴 데이터를 다시 가져옴
  },
  { immediate: true }
);
