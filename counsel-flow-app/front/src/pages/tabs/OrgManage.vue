...
import orgManageApi from "src/js/api/orgManageApi";
...
const columns = [{ title: "조직명", field: "name" }];
const orgStore = useOrganizationStore();
const permissionRoleStore = usePermissionRoleStore();

const canGetOrganization = computed(() => permissionRoleStore.hasPermission(FlowRoleCode.ORGANIZATION_ROLE.READ)); // 권한 확인
...
////////// CRUD API //////////
function getAllOrg() {
  if (canGetOrganization.value) {
    orgManageApi
      .getOrg()
      .then((response) => {
        const sortedData = sortWithChildren(response.data);
        formattedTreeData.value = transformTreeData(sortedData);
        org.value = addIdField(sortedData);
        resetDepthDatas();
        depth1.value = org.value;
      })
      .catch((error) => {
        handleApiError(error);
      });
  } else {
    showAlert("조직 조회 권한이 없습니다.");
  }
}

function getOrgByEntityId(entityId) {
  return orgManageApi
    .getOrg(entityId)
    .then((response) => {
...
async function deleteOrg({ depth, entityId }) {
  if (!entityId) {
  ...
  if (confirmed) {
    orgStore
      ...
      .then((response) => {
        if (response.status === 200) {
          if (canGetOrganization.value && depth === 1) {
            getAllOrg();
          } else if (canGetOrganization.value && depth === 2) {
            handleWhenIsDepth2();
          } else if (canGetOrganization.value && depth === 3) {
            handleWhenIsDepth3();
          } else {
            depth1.value = [];
          }
...
function handleDeleteDepth1(entityId) {
  deleteOrg({ depth: 1, entityId });
}

function handleDeleteDepth2(entityId) {
  deleteOrg({ depth: 2, entityId });
}

function handleDeleteDepth3(entityId) {
  deleteOrg({ depth: 3, entityId });
}
...
function handleSaveDepth({ depth, newRows, modifiedRows }) {
  ...
  const preparedNewRows = newRows.map((item, index) => {
    ...
    if (depth > 1) {
      baseObject.parent = { entityId: selectedParentEntityId };
    }
  ...
  const preparedModifiedRows = modifiedRows.map((item) => {
    ...
    if (depth > 1) baseObject.parent = { entityId: item.parentEntityId };
    ...
    orgStore
      ...
      .then((saveResponse) => {
        ...
        if (saveResponse.status === 200 && preparedModifiedRows.length) {
          return orgStore.update(preparedModifiedRows);
        }
      ...
      <table class="full-width a-table">
        ...
        <tbody>
          <tr>
            <td>
              <div class="row">
                <a-checkbox v-model="useName" label="조직명" />
              </div>
            </td>
            <td>
              <div class="row">
